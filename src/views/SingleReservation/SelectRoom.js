...
const SelectRoom = ({ stepper }) => {
    ...
    const guestDetail = async () => {
        setDropdownLoader(true)
        try {
            const guestDetailBody = { LoginID: localStorage.getItem('user-id'), Token: "123", Seckey: "abc", SearchPhrase: null, Event: "select" }
    ...
    const mealsDetail = async () => {
        try {
            const mealsBody = { LoginID: localStorage.getItem('user-id'), Token: "123", Seckey: "abc", Event: "select" }
            const response = await axios.post(`/getdata/bookingdata/mealdetails`, mealsBody)
